...
%%%%%%%%%%%%%%%%%% nu_t field CALCULATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dudx=(u(2:end,:)-u(1:end-1,:))/dx;
    dvdy=(v(:,2:end)-v(:,1:end-1))/dy;
    dudy=((ug(2:end-1,3:end)+ug(1:end-2,3:end))-(ug(2:end-1,1:end-2)+ug(1:end-2,1:end-2)))/(4*dy);
    dvdx=((vg(3:end,2:end)+vg(3:end,1:end-1))-(vg(1:end-2,2:end)+vg(1:end-2,1:end-1)))/(4*dx);
    
    temp = 2*(dudx).^2 + 2*(dvdy).^2 + (dudy + dvdx).^2;
    nu_t = (Cs*d)^2 * temp.^0.5;
    vvt=nu_t + nu;
%%%%%%%%%%%%%%%%%% nu_t field CALCULATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%% PHI CALCULATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    phiRHS=-dt*CONV + dt*(d2phidx2 + d2phidy2).*vvt; %is this negative?
    ...
    dphi_ss=zeros(nx,ny);
    for j = 1:ny
        Cphix = vvt(:,j)*dt/(2*dx^2);
        main_temp = (1+2*Cphix(2:end)).*ones(nx-1,1);
        lower_temp = -Cphix(2:end).*ones(nx-1,1);
        %lower_temp(end)=(-2*Cphix); %this is for neumann condition
        main_temp(end)=(1+Cphix(end)); %this is for neumann condition
        upper_temp = -Cphix(2:end).*ones(nx-1,1);
...
% %Step 1: compute viscous, convective terms in u and v, then compute ustar    
    ...
    RHS1u(2:end,:) = 0.5*dt*(-3*H_u_new + H_u_old) + (dt)*(d2udx2 + d2udy2).*vvt;
    RHS1v(:,2:end-1) = 0.5*dt*(-3*H_v_new + H_v_old) + (dt)*(d2vdx2 + d2vdy2).*vvt(:,2:end);
...
%%%IMPOSE 0 SQUARE VELOCITY    
    ...
    for j = 1:ny
        ...
        lower_temp(end)=(-2*Cux(end)); %this is for neumann condition
...
%Step 4: update augmented domain 
    ...
    %toc
    if mod(iter,100)==0
        filename = ['phi_' 'Re_' num2str(Re) 'Iter_' num2str(iter,'%05d') '.mat'];
        save(filename , 'phi' , 't')
    end	
